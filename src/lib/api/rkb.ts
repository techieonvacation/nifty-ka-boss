...
export interface ChartDataPoint {
  time: number; // UTC timestamp for chart rendering
  open: number;
  high: number;
  low: number;
  close: number;
...
// REAL-TIME FIX: Cache configuration optimized for real-time trading data
const CACHE_DURATION = {
  RKB_DATA: 60 * 1000, // REDUCED: 1 minute for real-time chart updates (was 1 hour)
  DECISIONS: 60 * 1000, // REDUCED: 1 minute for real-time decision updates (was 30 minutes)
  NIFTY_MOVEMENT: 5 * 60 * 1000, // 5 minutes for movement data (sufficient for this data type)
};
    ...
    const requestPromise = (async () => {
      ...
      try {
        ...
        if (errorName === "AbortError" || errorMessage?.includes("aborted")) {
          console.warn(
            `Request aborted for ${url} - this is normal during rapid navigation`
          );
...
// REAL-TIME FIX: Fetch RKB data with optional cache busting for real-time updates
export async function fetchRkbData(
  bustCache: boolean = false
): Promise<RkbDataPoint[]> {
  try {
    const cacheKey = "rkb-data";

    // REAL-TIME FIX: Clear cache if bust requested (for critical real-time updates)
    if (bustCache) {
      cache.delete(cacheKey);
      console.log("🚀 Cache busted for real-time RKB data update");
    }
    ...
    if (errorMessage === "Request was cancelled") {
      console.log(
        "RKB data request was cancelled - this is normal during rapid navigation"
      );
...
// REAL-TIME FIX: Fetch decisions data with optional cache busting for real-time updates
export async function fetchDecisions(
  ...
  try {
    ...
    if (bustCache) {
      cache.delete(cacheKey);
      console.log("🚀 Cache busted for real-time decisions data update");
    }
    ...
    if (errorMessage === "Request was cancelled") {
      console.log(
        "Decisions data request was cancelled - this is normal during rapid navigation"
      );
      return []; // Return empty array instead of throwing
    }
...
export async function fetchNiftyMovements(): Promise<NiftyMovementData[]> {
  try {
    ...
    if (errorMessage === "Request was cancelled") {
      console.log(
        "Nifty movement data request was cancelled - this is normal during rapid navigation"
      );
      return []; // Return empty array instead of throwing
    }
...
export function convertRkbDataToChartData(
  rkbData: RkbDataPoint[]
): ChartDataPoint[] {
  return rkbData
    .filter(
      (item) =>
    ...
    .map((item) => {
      // DATETIME FIX: Parse datetime string assuming IST and ensure correct chart display
      let time: number;
      try {
        // DATETIME FIX: Handle IST timezone conversion properly for chart time axis
        let dateStr = item.datetime;

        // If no timezone info, assume it's IST (UTC+5:30)
        if (
          !dateStr.includes("Z") &&
          !dateStr.includes("+") &&
          !dateStr.includes("-") &&
        ...
        // Convert to UTC timestamp for chart
        time = Math.floor(date.getTime() / 1000);

        // DATETIME DEBUG: Log conversion details to verify chart time axis
        if (item === rkbData[rkbData.length - 1]) {
          const chartDisplayTime = new Date(time * 1000);
          console.log(`🕐 Chart Time Debug:`);
          console.log(`   API DateTime: "${item.datetime}"`);
          console.log(`   With IST: "${dateStr}"`);
          console.log(`   UTC Timestamp: ${time}`);
          console.log(
            `   Chart Will Show: ${chartDisplayTime.toLocaleString("en-IN", {
              timeZone: "Asia/Kolkata",
            })} (IST)`
          );
          console.log(
            `   Browser Local: ${chartDisplayTime.toLocaleString()} (${
      ...
      return {
        ...
        originalLowDate: item.LowDate, // Keep raw LowDate string (e.g., "Thu, 24 Jul 2025 10:15:00 GMT")
      };
    })
